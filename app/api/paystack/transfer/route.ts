  ...
  try {
    ...
    if (!professional?.bank_account_number || !professional?.bank_name) {
      return NextResponse.json({
        error: 'Professional has not added their bank details yet. Ask them to add their bank account in their profile settings.'
      }, { status: 400 })
    }
    ...
    const transferResponse = await fetch('https://api.paystack.co/transfer', {
      ...
      body: JSON.stringify({
        ...
        reason: `Payment for ${contract.job_id} on SurveyConnect`,
      }),
    })
