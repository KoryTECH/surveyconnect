...
export async function POST(request: NextRequest) {
	try {
      ...
      `,
		...
		// flow). Mirrors the milestone release path at line 232. Best-effort:
		// a missing audit row does not unwind the release, since the contract
		// is already marked paid and Paystack has sent the funds.
		try {
			const releaseServiceClient = createServiceClient();
			await releaseServiceClient.from("transactions").insert([
				{
					contract_id: contractId,
