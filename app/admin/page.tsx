...
import { IdCard, FileText, Info, CheckCircle2, Users, Briefcase, FileCheck, DollarSign } from "lucide-react";
...
export default function AdminDashboard() {
	...
	const [stats, setStats] = useState({
		...
		totalJobs: 0, totalContracts: 0, platformRevenue: 0, pendingVerifications: 0,
	});
	...
	useEffect(() => {
		const init = async () => {
			...
			if (!profile?.is_admin) { router.push("/dashboard/client"); return; }

			const [
				{ count: totalUsers }, { count: totalClients }, { count: totalProfessionals },
				{ count: totalJobs }, { data: contracts }, { count: pendingCount },
			] = await Promise.all([
			...
			setStats({
				totalUsers: totalUsers || 0, totalClients: totalClients || 0,
				totalProfessionals: totalProfessionals || 0, totalJobs: totalJobs || 0,
				totalContracts: (contracts || []).length, platformRevenue: revenue,
				pendingVerifications: pendingCount || 0,
			});

			const { data: pending } = await supabase
				.from("professional_profiles")
				.select(`id, profession_type, license_number, years_experience, id_document_url, license_url, verification_status, created_at, profiles(full_name, email, country)`)
	...
	const getProfessionLabel = (type: string) => {
		const labels: any = {
			land_surveyor: "Land Surveyor", gis_analyst: "GIS Analyst", drone_pilot: "Drone/UAV Pilot",
			cartographer: "Cartographer", photogrammetrist: "Photogrammetrist", lidar_specialist: "LiDAR Specialist",
			remote_sensing_analyst: "Remote Sensing Analyst", urban_planner: "Urban Planner",
			spatial_data_scientist: "Spatial Data Scientist", hydrographic_surveyor: "Hydrographic Surveyor",
			mining_surveyor: "Mining Surveyor", construction_surveyor: "Construction Surveyor",
			environmental_analyst: "Environmental Analyst", bim_specialist: "BIM Specialist", other: "Other",
		};
	...
	const handleViewDocument = async (pathOrUrl: string) => {
		...
		const { data, error } = await supabase.storage.from("verification-documents").createSignedUrl(path, 60 * 60);
		if (data?.signedUrl) { window.open(data.signedUrl, "_blank"); }
		else { console.error("Signed URL error:", error); alert("Could not load document."); }
	};
	...
	const handleVerify = async (professionalId: string, action: "verified" | "rejected", prof: any) => {
		...
		const { error } = await supabase.from("professional_profiles")
			.update({ verification_status: action }).eq("id", professionalId);

		if (error) { setMessage("Something went wrong."); setActionLoading(null); return; }
		...
		if (action === "verified" && prof.license_url) {
			...
			await supabase.storage.from("verification-documents").remove([licensePath]);
		}
		...
		<div className="min-h-screen bg-gray-50 dark:bg-gray-950 transition-colors duration-300">
			<nav className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-6 py-4 flex items-center justify-between">
				<div className="flex items-center gap-3">
					<img src="/logo.png" alt="SurveyConnectHub" className="h-10 w-auto" onError={(e) => (e.currentTarget.style.display = "none")} />
				...
				<div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
					...
					<div className="bg-white dark:bg-gray-900 rounded-2xl p-5 border border-gray-100 dark:border-gray-800">
						<div className="flex items-center gap-2 mb-2">
							...
							<p className="text-gray-500 dark:text-gray-400 text-xs">Total Jobs</p>
						</div>
					...
					<div className="bg-white dark:bg-gray-900 rounded-2xl p-5 border border-gray-100 dark:border-gray-800">
						<div className="flex items-center gap-2 mb-2">
							...
							<p className="text-gray-500 dark:text-gray-400 text-xs">Total Contracts</p>
						</div>
					...
					<div className="bg-white dark:bg-gray-900 rounded-2xl p-5 border border-gray-100 dark:border-gray-800">
						<div className="flex items-center gap-2 mb-2">
							...
							<p className="text-gray-500 dark:text-gray-400 text-xs">Platform Revenue</p>
						</div>
									...
									<div className="flex items-start justify-between gap-4 flex-wrap">
										<div className="flex-1">
											<div className="flex items-center gap-3 mb-3">
												...
												<div>
													<p className="font-semibold text-gray-900 dark:text-white">{prof.profiles?.full_name}</p>
