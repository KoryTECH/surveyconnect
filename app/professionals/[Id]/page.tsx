...
export default function ProfessionalProfilePage() {
	...
	useEffect(() => {
		const getData = async () => {
			...
			// If viewer is a client, fetch contracts with this professional
			// where payment has been released (eligible to review)
			if (viewerProfile?.role === "client") {
				...
				// Filter out contracts already reviewed
	...
	const handleSubmitReview = async () => {
		...
		if (!error) {
			...
			setReviews(reviewsData || []);
			setEligibleContracts((prev) => prev.filter((c) => c.id !== selectedContract));
			setRating(0);
			setComment("");
		...
		return (
			<div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex items-center justify-center">
				<div className="text-center">
					...
					<Link href="/professionals" className="text-green-600 hover:underline">
						...
						<div className="flex-1">
							...
							{avgRating && (
								<div className="flex items-center gap-2">
									<div className="flex">
										{[1, 2, 3, 4, 5].map((star) => (
											<span key={star} className={`text-lg ${Number(avgRating) >= star ? "text-yellow-400" : "text-gray-300 dark:text-gray-600"}`}>★</span>
										))}
									</div>
									<span className="text-sm font-semibold text-gray-900 dark:text-white">{avgRating}</span>
									<span className="text-xs text-gray-500 dark:text-gray-400">({reviews.length} review{reviews.length !== 1 ? "s" : ""})</span>
								</div>
							)}
						</div>
					</div>
				...
				{viewerRole === "client" && eligibleContracts.length > 0 && (
					<div className="bg-white dark:bg-gray-900 rounded-2xl p-6 border border-gray-100 dark:border-gray-800">
						...
						{eligibleContracts.length > 1 && (
							<div className="mb-4">
								<label className="text-sm text-gray-600 dark:text-gray-400 mb-1 block">Select Contract</label>
								<select
								...
								>
									{eligibleContracts.map((c: any) => (
										<option key={c.id} value={c.id}>{c.jobs?.title}</option>
									))}
								</select>
							</div>
						)}
						...
						<div className="mb-4">
							<label className="text-sm text-gray-600 dark:text-gray-400 mb-2 block">Rating</label>
						...
						<div className="mb-4">
							<label className="text-sm text-gray-600 dark:text-gray-400 mb-1 block">Review (optional)</label>
							<textarea
								value={comment}
						...
						{reviewSuccess && (
							<div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-3 mb-4">
								<p className="text-green-700 dark:text-green-400 text-sm font-medium">✅ Review submitted successfully!</p>
							</div>
						)}
							...
							{reviews.map((review) => (
								<div key={review.id} className="border-b border-gray-100 dark:border-gray-800 last:border-0 pb-4 last:pb-0">
									<div className="flex items-center justify-between mb-2">
										...
										<div className="flex">
											{[1, 2, 3, 4, 5].map((star) => (
												<span key={star} className={`text-sm ${review.rating >= star ? "text-yellow-400" : "text-gray-300 dark:text-gray-600"}`}>★</span>
											))}
										</div>
									</div>
									{review.comment && (
