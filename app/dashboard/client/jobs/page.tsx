      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">{jobs.length} job{jobs.length !== 1 ? 's' : ''} posted</p>
          </div>
          <Link
            href="/jobs/post"
        ...
        {jobs.length === 0 ? (
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-12 text-center border border-gray-100 dark:border-gray-800">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-6">Post your first job to start receiving applications</p>
            <Link
              href="/jobs/post"
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 text-xs text-gray-400 dark:text-gray-500 flex-wrap">
                      <span>📍 {[job.location_city, job.location_country].filter(Boolean).join(', ') || 'Remote'}</span>
                  ...
                  <div className="text-right shrink-0 space-y-2">
                    <div className="bg-green-50 dark:bg-green-900/20 rounded-xl px-4 py-2 text-center">
                      <p className="text-2xl font-bold text-green-600 dark:text-green-400">{job.applications_count || 0}</p>
                      <p className="text-xs text-green-700 dark:text-green-500">applicant{job.applications_count !== 1 ? 's' : ''}</p>
                    </div>
                    <Link
